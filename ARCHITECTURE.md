...
Sinh 4 ảnh trên Mac hiện tại có thể mất **~14 phút** (ước tính từ benchmark ~212s/panel). Nếu Angular → NestJS → Orchestrator giữ HTTP connection chờ kết quả:
...
#### Rủi ro #3 — Nhất quán nhân vật giữa 4 khung (CHƯA CÓ)

Đây là **core value** của comic, không phải nice-to-have. Hiện image-ai mới sinh **1 panel độc lập**. Cần:
- story-ai output `character_bible` (mô tả cố định)
- Panel 1 làm reference image cho panel 2–4 (IP-Adapter / InstantID)
- Prompt template giữ character ID xuyên suốt

Không giải quyết được điểm này thì 4 khung sẽ là 4 cảnh không liên quan — sản phẩm mất ý nghĩa.
...
- [ ] Character consistency: IP-Adapter panel 1 → 2,3,4
...
| Điểm yếu lớn nhất hiện tại? | **be-comic, story-ai, orchestrator đều trống** — image-ai đi trước quá xa so với pipeline |
| Điểm mạnh lớn nhất? | **image-ai production-ready pattern** (gRPC + Celery + cache + MinIO) |
