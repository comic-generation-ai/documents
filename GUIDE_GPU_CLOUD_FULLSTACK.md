...
> Kết nối qua VS Code **Remote-SSH**. Bỏ qua `fe-comic` — test qua Swagger
> `/docs` của `be-comic` là đủ để kiểm tra full luồng BE.
...
## Bước 2 — Clone code (4 repo, bỏ fe-comic)
...
## Bước 7 — Chạy 5 tiến trình (mỗi cái 1 tab Terminal riêng, để nguyên chạy)
...
## Bước 8 — Forward port, test

Panel dưới VS Code → tab **PORTS** → forward port `3000` (be-comic) và `9000`
(MinIO, để xem ảnh sinh ra). Mở `http://localhost:3000/docs` trên máy local —
test theo đúng thứ tự:

1. Đăng ký/đăng nhập (nếu API yêu cầu auth)
2. `POST /api/projects`
3. `POST /api/generation-jobs`
4. Poll `GET /api/generation-jobs/{jobId}` tới khi status = SUCCESS
5. `GET /api/frames?projectId=<id>` — lấy URL ảnh từng panel (MinIO)
